...
import { db } from "./db";
import {
  users, registerSchema, loginSchema, passwordResetTokens, forgotPasswordSchema,
  resetPasswordSchema, emailVerificationTokens, verifyEmailSchema, resendVerificationSchema,
} from "@shared/schema";
...
async function sendVerificationForUser(user: User): Promise<void> {
  const baseUrl = trustedBaseUrl();
  if (!baseUrl) throw new Error("No trusted origin is configured for verification links");
  ...
  try {
    await sendEmailVerificationEmail(user.email, `${baseUrl}/verify-email?token=${token.rawToken}`);
  } catch (error) {
...
export function registerAuthRoutes(app: Express) {
  ...
  app.post("/api/auth/resend-verification", recoveryLimiter, async (req, res) => {
    try {
      ...
      const genericResponse = {
        ok: true,
        message: "If that account still needs verification, we've sent a new link.",
      };
      const user = await getUserByEmail(parsed.data.email);
  ...
  app.post("/api/auth/verify-email", recoveryLimiter, async (req, res) => {
    try {
