...
**Proxy relay escape hatch:** An IP block only affects OUR IP — the same request from a ScrapingBee plain proxy (render_js=false, ~1 credit) usually works. The fetcher relays blocked-host requests through it (attempt-counted per-host budget), and the crawl loop must skip Puppeteer for blocked hosts (the browser can't use the relay). Caveat: ScrapingBee's free tier is 1000 calls/month and the Cloudflare stealth fallback burns 75 credits per call, so quota exhaustion (HTTP 401 "Monthly API calls limit reached") is the common failure — a process-wide flag then disables the relay and the blocked message tells the user the proxy quota is out.

**Pacing won't fix this firewall:** Compared against a SiteSucker run from the user's residential IP: it fetched ~4 files/sec for 160 files with zero blocks, while our sequential crawl (~1 request per 2s, no Puppeteer, probe=static) got banned after ~8 requests. The trigger is datacenter-IP reputation, not request rate — slowing down doesn't help; only alternate IPs (proxy relay) do. Block lifted in roughly 16 hours both times.
