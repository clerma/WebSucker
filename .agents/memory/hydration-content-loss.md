...
**How to apply:** The scraper has two guards in the Puppeteer HTML path: (1) a hydration-settle loop polling `body.innerText` length + iframe count until stable (bounded cap; exact seconds are tunable — check code) before capture; (2) a content-loss guard that, when the rendered capture looks thin, plain-fetches the URL and keeps the raw server HTML if it scores much richer (text/img heuristics). Never remove these when refactoring the render pipeline. When debugging "missing content" complaints, always diff the saved page against a plain fetch of the live URL first — it instantly shows whether the loss happened at render time.

**Wix raw-HTML caveat:** A Wix raw SSR page can contain all text and images yet still be visually incomplete because entry-effect CSS hides components until runtime adds `data-motion-enter="done"`. Prefer the settled rendered DOM while it retains the real Wix layout; only select raw when text and images show combined severe loss. Any Wix page saved for offline use must freeze motion components at their completed state.

**Why:** A real backup preserved the hero background/title but hid the header, logo, tagline, copy, button, and later sections at opacity zero. The content was present; the raw snapshot had retained Wix's pre-animation state, and Wix runtime could not reliably finish it from a local origin.
